...
public class XPathNodeListProxyPrototype extends KrollProxyPrototype
{
	private static final long serialVersionUID = -6533962430645191161L;

	private static final String TAG = "XPathNodeListProxyPrototype";
	private static final String CLASS_TAG = "XPathNodeListProxy";
	private static XPathNodeListProxyPrototype xPathNodeListProxyPrototype;


	public static XPathNodeListProxyPrototype getProxyPrototype()
	{
		return xPathNodeListProxyPrototype;
	}

	public static void dispose()
	{
		Log.d(TAG, "dispose()", Log.DEBUG_MODE);
		xPathNodeListProxyPrototype = null;
	}

	public XPathNodeListProxyPrototype()
	{
		if (xPathNodeListProxyPrototype == null && getClass().equals(XPathNodeListProxyPrototype.class)) {
			xPathNodeListProxyPrototype = this;
	...
	{
		if (this == xPathNodeListProxyPrototype) {
	...
	{
		return KrollProxy.createProxy(XPathNodeListProxy.class, getRhinoObject(), args, creationUrl);
	}
	...
	public Object item(Context context, Scriptable thisObj, Object[] args)
	{
		...
		try {
		XPathNodeListProxy proxy = (XPathNodeListProxy) ((Proxy) thisObj).getProxy();
		if (args.length < 1) {
	...
	public Number getter_length()
	{
		...
		XPathNodeListProxy proxy = (XPathNodeListProxy) getProxy();
...
// #/string_id_map#
		...
		Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		while (thisObj != null && !(thisObj instanceof XPathNodeListProxyPrototype)) {
			thisObj = thisObj.getPrototype();
		}

		XPathNodeListProxyPrototype proxy = (XPathNodeListProxyPrototype) thisObj;
		int id = f.methodId();
		switch (id) {
			case Id_constructor:
...
// #/string_id_map#
	...
	{
		XPathNodeListProxyPrototype proxy = this;
		while (start != null && !(start instanceof XPathNodeListProxyPrototype)) {
			start = start.getPrototype();
		}

		if (start instanceof XPathNodeListProxyPrototype) {
			proxy = (XPathNodeListProxyPrototype) start;
		}
