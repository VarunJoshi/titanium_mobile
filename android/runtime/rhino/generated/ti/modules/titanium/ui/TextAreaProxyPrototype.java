...
public class TextAreaProxyPrototype extends TiViewProxyPrototype
{
	...
	{
		if (this == textAreaProxyPrototype) {
	...
	{
		return KrollProxy.createProxy(TextAreaProxy.class, getRhinoObject(), args, creationUrl);
	}
	...
	public void setSelection(Context context, Scriptable thisObj, Object[] args)
	{
		...
		try {
		TextAreaProxy proxy = (TextAreaProxy) ((Proxy) thisObj).getProxy();
		if (args.length < 2) {
...
// #/string_id_map#
		...
		Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		while (thisObj != null && !(thisObj instanceof TextAreaProxyPrototype)) {
			thisObj = thisObj.getPrototype();
		}

		TextAreaProxyPrototype proxy = (TextAreaProxyPrototype) thisObj;
		int id = f.methodId();
		switch (id) {
			case Id_constructor:
...
// #/string_id_map#
	...
	{
		TextAreaProxyPrototype proxy = this;
		while (start != null && !(start instanceof TextAreaProxyPrototype)) {
			start = start.getPrototype();
		}

		if (start instanceof TextAreaProxyPrototype) {
			proxy = (TextAreaProxyPrototype) start;
		}
